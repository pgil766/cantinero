...
6. **Prohibido** usar como LLM del agente un modelo comercial en la nube (OpenAI, Anthropic, Gemini, etc.).
   El LLM y los embeddings corren **localmente**, ambos servidos por **Ollama**.
...
| `backend` | propia (`backend/Dockerfile`) | No (vía Caddy `/api`) | — |
...
- [x] **T2.5** — Bitácora: RAM y latencia de `qwen2.5:3b` en CPU y en GPU. *(Hecho: ver la bitácora del 2026-09-23 "Mediciones del modelo local".)*

### Fase 3 — Ingesta y base vectorial (M5)

- [x] **T3.1** — Reunir el *seed* ([3.3](#33-base-de-conocimiento-inicial-seed)): recetas IBA en CSV (ml + oz), artículos de destilados y técnicas en MD, al menos un PDF y un DOCX. Preparar aparte `data/demo/guia_destilados_colombianos_viche.pdf`. ✔ 20–40 archivos, fuentes listadas en el README (ver `docs/fuentes.md`), ninguna mención del viche en el *seed* y cada pregunta `responder` de `eval/preguntas.yaml` cubierta por algún documento (ajustar `debe_contener` a lo que dicen las fuentes). *(Hecho por un subagente recopilador y verificado con los extractores reales: 37 archivos (94 recetas IBA en CSV con medidas verificadas contra la página oficial, 34 artículos de Wikipedia con atribución CC BY-SA, un PDF y un DOCX propios), 785 fragmentos, cero menciones del viche en el seed. Demo: `data/demo/guia_destilados_colombianos_viche.pdf`. Fuentes y exclusiones en `docs/fuentes.md`; scripts de regeneración en `data/tools/`. Las 27 preguntas `responder` recuperan su documento correcto en el primer puesto.)*
- [x] **T3.2** — `services/ingestion/loaders.py`: PDF, TXT, MD, CSV y DOCX con metadatos. ✔ Pruebas con un archivo de cada tipo (`tests/fixtures/`). *(Hecho: `loaders.py` con `pypdf` y `docx2txt` (Python puro, sin bloqueos de Smart App Control); PDF por página, MD por encabezado con su ruta, CSV una fila = una sección atómica con la URL en metadatos, errores 415/422 legibles; 13 pruebas con PDF y DOCX generados en memoria.)*
- [x] **T3.3** — `splitter.py`: una receta por fragmento (CSV por fila, MD por encabezado) y `RecursiveCharacterTextSplitter` para texto largo. ✔ Prueba: ninguna receta queda partida y no hay fragmentos vacíos. *(Hecho: `splitter.py`; las secciones atómicas nunca se parten, el encabezado se antepone a cada fragmento y se descartan los de menos de 30 caracteres; 5 pruebas.)*
- [x] **T3.4** — `vectorstore/store.py`: insertar fragmentos, borrar por `document_id` y buscar con filtro "global + propios" y puntaje normalizado. ✔ Prueba de ida y vuelta: insertar → buscar → borrar. ⚠️ Con el índice HNSW, pgvector filtra **después** de recorrer el índice (unos 40 candidatos con `hnsw.ef_search` por defecto): con el filtro por usuario puede devolver menos de `top_k`. Usar `SET LOCAL hnsw.iterative_scan = relaxed_order` (pgvector 0.8) dentro de la transacción de búsqueda y probarlo. *(Hecho: `vectorstore/store.py` con `iterative_scan`; 8 pruebas, incluido un control negativo que muestra que sin `iterative_scan` el usuario recibe 0 de 3 resultados cuando 300 fragmentos privados de otro usuario acaparan el índice.)*
- [x] **T3.5** — `pipeline.py`: validar → extraer → dividir → *embed* → guardar (transacción) → actualizar `documents`. ✔ Un PDF real termina en `ready` con `chunk_count > 0`. *(Hecho: `pipeline.py` valida extensión, tamaño y que el contenido coincida con la extensión (no confía en el MIME del navegador); calcula los embeddings fuera de la transacción; ante cualquier error deja `failed` con un motivo legible y sin fragmentos. 12 pruebas.)*
- [x] **T3.6** — Deduplicación por `sha256` por usuario. ✔ Subir el mismo archivo dos veces da 409. Traducir `UniqueViolation` (dos subidas simultáneas) a 409, nunca a 500. Un documento en `failed` **no** bloquea el reintento (índice parcial `WHERE status <> 'failed'`): borrar la fila fallida al reintentar. *(Hecho: `DuplicateDocumentError` (409) al traducir `UniqueViolation`; el mismo archivo se permite para otro usuario; un intento `failed` se reemplaza al reintentar.)*
- [x] **T3.7** — `scripts/ingest_seed.py` (documentos globales) y `scripts/reindex.py`. ✔ Base poblada; conteo en el log. *(Hecho: el seed se carga en 22.7 s con GPU (37 archivos, 785 fragmentos); `--replace` recarga los globales; `reindex.py` recalcula los embeddings desde el texto guardado.)*
...
| R16 | *Smart App Control* de Windows bloquea binarios de Python sin firma (pasó con `scikit-learn`) | Media | Media | Preferir dependencias puras o firmadas; si algo se bloquea, correr esa parte en Docker. **No** apagar Smart App Control (no se puede volver a activar) |
| R17 | El sitio de la IBA no publica una licencia abierta para sus recetas | Baja | Media | Se usan solo los datos (ingredientes, medidas, método), traducidos y citados receta por receta; si se objeta, reemplazar por recetas de Wikipedia (CC BY-SA). Mencionarlo en el README |
