...
- [ ] **T0.4** — Crear el repositorio **`cantinero`** en GitHub (público), agregar a ambos integrantes, `git init`, `.gitignore` (Python, Node, `.env`, `.venv`, `node_modules`, `data/uploads`, `trabajo agente.pdf`), README mínimo, este AGENTS.md y `docs/bitacora.md`. ✔ Primer commit subido. *(Avance: repositorio local listo con el primer commit; falta subirlo al repositorio nuevo de GitHub e invitar a Emanuel como colaborador.)*
